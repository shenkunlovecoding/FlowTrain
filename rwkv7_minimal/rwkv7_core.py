...
# while replacing fused CUDA kernels / Lightning / DeepSpeed with a pure PyTorch reference path.
########################################################################################################
...
try:
    from .cuda_ops import can_use_rwkv7_cuda, rwkv7_recurrence_cuda
    from .tilelang_block import rwkv7_block_tilelang
except ImportError:  # Allows running this file directly from rwkv7_minimal/.
    from cuda_ops import can_use_rwkv7_cuda, rwkv7_recurrence_cuda
    from tilelang_block import rwkv7_block_tilelang
...
class RWKV7Config:
    ...
    cuda_kernel_variant: str = "h100"
    cuda_chunk_len: int = 16

    def __post_init__(self) -> None:
        if self.cuda_kernel_variant not in {"h100", "base", "head128", "tilelang_block"}:
            raise ValueError(f"unknown RWKV7 CUDA variant: {self.cuda_kernel_variant}")
...
class RWKV7TimeMix(nn.Module):
    ...
    def forward(self, x: torch.Tensor, v_first: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.config.use_cuda_kernel and can_use_rwkv7_cuda(r, self.head_size, self.config.cuda_chunk_len):
            recurrence_variant = self.config.cuda_kernel_variant
            if recurrence_variant == "tilelang_block":
                recurrence_variant = "head128" if self.head_size == 128 else "h100"
            x = rwkv7_recurrence_cuda(
                r,
                ...
                kk * a,
                self.head_size,
                chunk_len=self.config.cuda_chunk_len,
                variant=recurrence_variant,
            )
        else:
...
class RWKV7Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, v_first: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if self.config.cuda_kernel_variant == "tilelang_block":
            return rwkv7_block_tilelang(self, x, v_first)
        return self._forward_standard(x, v_first)

    def _forward_standard(self, x: torch.Tensor, v_first: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if self.layer_id == 0:
            x = self.ln0(x)
